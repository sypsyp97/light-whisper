...
pub async fn transcribe(
    ...
    if api_key.is_empty() {
        return Err(AppError::FunASR("GLM-ASR API Key 未配置".into()));
    }
    ...
    let file_part = reqwest::multipart::Part::bytes(audio_data)
        .file_name("audio.wav")
        .mime_str("audio/wav")
        .map_err(|e| AppError::FunASR(format!("构建 multipart 失败: {}", e)))?;
    ...
    if let Some(words) = hot_words {
        let json = serde_json::to_string(&words)
            .map_err(|e| AppError::FunASR(format!("序列化热词失败: {}", e)))?;
    ...
    let resp = state
        ...
        .send()
        .await
        .map_err(|e| AppError::FunASR(format!("GLM-ASR 请求失败: {}", e)))?;

    let status = resp.status();
    let body = resp
        .text()
        .await
        .map_err(|e| AppError::FunASR(format!("读取 GLM-ASR 响应失败: {}", e)))?;
    ...
    let parsed: GlmResponse = serde_json::from_str(&body)
        .map_err(|e| AppError::FunASR(format!("解析 GLM-ASR 响应失败: {}", e)))?;
