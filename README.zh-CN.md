...
| **需要联网** | ❌ 本地引擎离线；GLM-ASR 和 AI 润色需 API | ✅ 始终 |
| **ASR 引擎** | SenseVoice + Faster Whisper + GLM-ASR（可切换） | 自研云端引擎 |
...
| **依赖** | Python + GPU/CPU | Python + GPU/CPU | 仅需 API Key |
...
| **Rust 服务** | `src-tauri/src/services/` — funasr_service, glm_asr_service, audio_service, ai_polish_service, llm_provider, profile_service |
...
默认 F2，若被其他程序占用，在设置 > 说话热键中更换，支持任意组合键（如 `Ctrl+Win+R`）。
