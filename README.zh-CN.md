...
从 [Releases](https://github.com/sypsyp97/light-whisper/releases/latest) 页面下载 `*_x64-setup.exe` 安装包，运行安装即可。所有依赖已内置，无需安装 Python 或编译工具。ASR 模型会在首次使用时自动下载（约 1–1.5 GB）。
...
**四个 ASR 引擎**<br>
SenseVoice（中/英/日/韩/粤）和 Faster Whisper（99+ 语言）完全本地运行，支持 CUDA 加速；GLM-ASR 和阿里 DashScope/Qwen ASR 在线调用，只需填入 API Key，无需 Python 和显卡。

**AI 润色**<br>
可选 LLM 后处理：修正同音字、标点、口头禅；自动检测前台应用适配语气。可先显示 ASR 原文，再在 LLM 返回后更新为润色结果；结果卡片会显示 ASR、AI 和总耗时。内置 OpenAI / DeepSeek / Cerebras / SiliconFlow 预设，支持 OpenAI 兼容和 Anthropic API 格式。

**自适应学习**<br>
只学习结构化纠错和术语；手动删除的脏热词会进入黑名单，不再自动长回来。

**字幕悬浮窗**<br>
透明浮窗实时显示听写状态、ASR 原文预览、润色完成状态和助手结果。
...
| 引擎 | 运行方式 | 适合场景 | 语言 / 模型 | 配置 |
|:--|:--|:--|:--|:--|
| **SenseVoice** | 本地 Python 引擎 | 默认听写、低延迟本地使用 | 中 / 英 / 日 / 韩 / 粤 | 下载 SenseVoiceSmall + VAD 模型 |
| **Faster Whisper** | 本地 Python 引擎 | 更广的语言覆盖 | large-v3-turbo-ct2，99+ 语言 | 下载 Whisper 模型 |
| **GLM-ASR** | 在线 API | 免本地模型下载的云端 ASR | `glm-asr-2512` | API Key + 区域端点 |
| **阿里 DashScope** | 在线 API | DashScope 上的 Qwen ASR / Omni 模型 | 默认 `qwen3-asr-flash`；可在设置里刷新模型列表 | API Key + 区域 + 模型 |
...
> [!TIP]
> **在线 ASR 用户：** 如果只使用 GLM-ASR 或阿里 DashScope/Qwen ASR，只需安装 Rust、Node.js、pnpm 即可，无需 Python、uv 和显卡。构建后在设置中填入 API Key 即可使用。
...
└──────────────┘                └──────┬───────┘                │  Faster Whisper │
                                       │                        └─────────────────┘
                                       ├─── HTTP ──► 在线 ASR API（GLM-ASR / 阿里 DashScope）
...
| **Rust 服务** | `src-tauri/src/services/` — funasr_service, glm_asr_service, alibaba_asr_service, audio_service, assistant_service, ai_polish_service, llm_client, llm_provider, profile_service, screen_capture_service, web_search_service, download_service |
