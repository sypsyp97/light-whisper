...
AI 润色、屏幕感知和助手联网搜索均提供 **关闭／开启／自动**。只有自动档使用 Jev 判断是否需要处理，在 **自动判断 → Jev** 中统一选择 TypeSafe（官方）、OpenRouter 或 Vercel 并填写 API Key。缺少密钥、超时或判断不确定时沿用原处理流程；翻译和明确的编辑操作仍会执行。

联网搜索支持免密钥的 Bing 摘要搜索，以及提供有限免费额度、可选填 API Key 的 Exa MCP。免费服务可能限流或要求浏览器验证；应用会明确提示，并在没有网页来源的情况下继续回答。

还可单独开启 Jev 纠错规则审核和润色原意检查。原意检查在后台运行，只提醒可能的改意，不延迟或替换输出；你亲自确认的纠错规则会保留。
